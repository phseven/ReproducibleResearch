...
# No plotting, just the labels, axes

pc[, plot(strptime(paste(DT, TM), format="%d/%m/%Y %T"), 
    SM1, type="n", 
    xlab="", ylab="Energy sub metering") ]

# Put the legend in top right corner
...
# Black lines for Sub meter 1 (SM1)

pc[, lines(strptime(paste(DT, TM), format="%d/%m/%Y %T"), 
    SM1, type="l", col="black")]

# Red lines for Sub meter 2 (SM2)

pc[, lines(strptime(paste(DT, TM), format="%d/%m/%Y %T"), 
    SM2, type="l", col="red")]

# Blue lines for Sub meter 3 (SM3)
